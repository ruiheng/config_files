...
7. if `Per-task review: skip`, run workspace prepare for this task, then run `adwf planner-closeout-batch` directly with the recorded `task_branch`, `integration_branch`, `worker_workspace`, `planner_workspace`, `task_id`, and task dir before marking the task done
