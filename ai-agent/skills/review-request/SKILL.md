...
- if no visible local work remains, wait once for the review reply with blocking `mailbox_recv` using timeout `2h`
- if the receive times out, report that no review reply has arrived yet; do not inspect or repair the reviewer session
