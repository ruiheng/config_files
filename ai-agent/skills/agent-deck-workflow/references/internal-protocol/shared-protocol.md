...
- if a shell helper requires a real body file, write it under the current agent workdir's `.agent-artifacts/mailbox/`; do not use target/worker workdirs or global temp dirs such as `/tmp`
