...
- if no visible local work remains, wait once for the advisory report with blocking `mailbox_recv` using timeout `2h`
- if the receive times out, report that no advisory report has arrived yet; do not inspect or repair the reviewer session
