...
- notification nudges are optional acceleration; idle sessions should block on mailbox receive
...
7. Group subscriber notification may wake the moderator; idle moderator sessions should still block on mailbox receive. If normal `check-agent-mail` finds no personal delivery, it may hand off to `roundtable` moderator group check when active roundtable context is present.
...
- coder/reviewer/architect progress is asynchronous and may take unbounded time; planner must not expect cross-session dispatch to finish within a short receive timeout
- after cross-session dispatch, planner should do independent non-interfering work when available; otherwise wait with blocking `mailbox_recv`
- use a single `2h` receive timeout when idle; if it times out, report that no mail/reply has arrived yet
- a receive timeout is not failure evidence; do not inspect or repair another agent's session because of it
...
3. Queue mail first. Best-effort nudges may wake non-local targets, but correctness comes from blocking mailbox receive.
   Newly created or restarted targets should use the same mailbox receive path as any other target.
