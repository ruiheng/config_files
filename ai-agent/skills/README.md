...
- notification nudges are optional acceleration; idle sessions should wait with `mailbox_wait`, then claim with `mailbox_recv`
...
7. Group subscriber notification may wake the moderator; idle moderator sessions should still use `mailbox_wait`, then `mailbox_recv`. If normal `check-agent-mail` finds no personal delivery, it may hand off to `roundtable` moderator group check when active roundtable context is present.
...
- after cross-session dispatch, planner should do independent non-interfering work when available; otherwise wait with `mailbox_wait`, then claim with `mailbox_recv`
- use a single `2h` wait timeout when idle; if it times out, report that no mail/reply has arrived yet
...
3. Queue mail first. Best-effort nudges may wake non-local targets, but correctness comes from `mailbox_wait` plus `mailbox_recv`.
